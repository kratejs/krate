...
pub fn build_index(documents: Vec<Document>) -> Result<Index, Box<dyn std::error::Error>> {
	...
	for (doc_index, doc) in documents.iter().enumerate() {
		...
		// Krate raises the upstream budgets (5 singles / 3 doubles) so that
		// documentation pages — which use many domain-specific single words —
		// are findable without manually specifying `keywords` in frontmatter.
		let mut single_word_budget = 15;
		let mut double_word_budget = 6;
